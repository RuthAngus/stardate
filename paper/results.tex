...
\gaia\ $G$, $G_{BP}$ and $G_RP$ were generated, without noise, from these
...
parameter space.
We found that the following set of initial parameters worked well for FGK
dwarfs but sacrificed some accuracy in subgiant ages: $EEP = 330$, $A = 9.56$
Gyr, $F = -0.05$, $D = 269$ pc and $A_V = 0.0$.

%   - The results
Figure \ref{fig:simulation_results} shows the results of running \sd\ on
simulated stars.
The true ages are plotted vs. the ages predicted using \sd\: a combination of
gyrochronology and isochrone fitting in color, with the ages predicted using
isochrone fitting only for the same stars plotted in light grey.
...
these same types of stars that have ceased magnetic braking (Ro $>$ 2), hot
stars ($B-V < 0.45$), cool stars ($B-V > 1.4$) and evolved stars (EEP $>$
454).
...
For the whole sample, including stellar rotation periods results in an overall
age precision of 10\% (down from 28\%), a median error of 340 Myrs (down from
1.3 Gyrs), and a relative error of 4\% (down from 23 \%).
The ages of FGKM dwarfs and subgiants are 3$\times$ more precise and 6$\times$
more accurate, on average, when their rotation periods are used to infer their
ages.
