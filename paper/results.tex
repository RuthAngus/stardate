...
grey.
\racomment{
    The three panels show the results for three different types of stars: FGK
dwarfs (\gcolor\ $<$ 2.2), M dwarfs (2.2 $<$ \gcolor) and subgiants (420 <
EEP).
...
% (EEP $>$ 454).
The FGK dwarfs show an improvement in age precision and accuracy when modeled
with isochrones and gyrochronology.
The median age precision for this group (defined as the standard deviation of
the posterior as a percentage of the median age) was 12\% when using a
combination of isochrones and gyrochronology, and 43\% using isochrone fitting
alone.
The age RMS was 1.2 Gyr for isochrones plus gyrochronology and 2.9 Gyr for
isochrones only.
The precision of M dwarf ages changed very little when their rotation periods
were included, with precision improving from 56\% to 38\% and RMS from 4.5 Gyr
to 4.3 Gyr.
This very small improvement is entirely driven by the early M dwarfs.}
\begin{figure}
  \caption{
...
\sd\ than with isochrone fitting alone.
...
An interesting side-effect of fitting isochrones with rotation periods is an
increased precision in EEP as well as age.
...
period {\it is a function of HRD position}.
As shown in equation \ref{eqn:gyro}, rotation period does not just depend on
color and age, but also on EEP and [Fe/H].}
\jegin{figure}
  \caption{
    Inferred EEP for the simulated star sample with isochrone fitting only
(left panel) and isochrones plus gyrochronology (right panel).
    Rotation periods provide information about position on the HRD via
equation \ref{eqn:gyro} and therefore improves all stellar parameters, not
just age.
Mass is determined by EEP, age and [Fe/H], so improvements in EEP and age will
improve stellar mass estimates.
}
  \centering
