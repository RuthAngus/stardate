...
early M stars.
We also fit a straight to the late M dwarfs (\gcolor\ $>$ 2.7).
...
\begin{equation}
    \log_{10}(P_{rot}) = f(A)g(C) \nonumber
\end{equation}
\begin{equation}
    = 0.65\log_{10}A -38.96 + 28.71C -4.92C^2 + 0.72C^3 -4.72C^4
\end{equation}
where C is $\log_{10}(G_{BP}-G_{RP})$, for stars with \gcolor\ $<$ 2.7 and
\begin{equation}
    \log_{10}(P_{rot}) = f(A)h(C) \nonumber
\end{equation}
\begin{equation}
     = 0.65\log_{10}(A) -13.56 + 0.94C,
\end{equation}
...
\citet{wright2011}.
The rotation periods of hot stars (\gcolor\ $<$ 0.56) are fast and usually do
...
envelope needed to generate strong magnetic fields.
Late M dwarfs with masses $\lesssim$ 0.3 M$_\odot$, temperatures $\lesssim$
3500 and \gcolor $\gtrsim$ 2.7 do not start magnetic braking until at least
after the age of Praesepe ($\sim$650 million years).
...
maximum values at 0.56, 2.5 and 454 for hot stars, cool stars and subgiants,
respectively.
