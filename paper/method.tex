...
\begin{figure}
  \caption{
    ...
    these data.
    The dashed line and shaded region shows the mean model and variance
    described in equation \ref{eqn:gyro}.
    The solid blue line shows the \citep{angus2015} gyrochronology relation.
...
\begin{table}[h!]
  ...
  \end{center}
\end{table}
\racomment{We calibrated a relation to Praesepe and the Sun alone and did not
include other clusters or asteroseismic stars in the fit for the following
reasons.
Firstly, different open clusters have slightly different period-color
relationships \citep{agueros2018, agueros2018b, curtis2018} and, given that we
use an age-color separable relation, adding in extra clusters is unlikely to
improve the calibration in this format.
We did not calibrate to asteroseismic stars because they are slightly evolved
and, because this gyrochronology relation does not include $\log g$, not
useful calibrators.
This new relation does not perfectly predict the rotation periods of stars at
all colors and ages, but it provides several improvements over previous
...
the rotational behavior of M dwarfs for the first time, and thirdly, it is
calibrated to \Gaia\ \gcolor\ color: the most widely available photometric
color index (as opposed to B-V).
...
convective stars which a long time to converge onto the \citet{skumanich1972}
braking law.
In addition, stars with Rossby numbers larger than around 2 do not show
Skumanich-like magnetic braking \citep{angus2015, vansaders2016,
vansaders2018}, so equation \ref{eqn:fgk_gyro} does not describe these stars.
...
In order to include these non-Skumanich type stars in our combined gyrochronal
and isochronal model, we designed a composite gyrochronology relation which
describes the mean and variance of the rotation period distribution across the
HRD.
...
\begin{itemize}
    \item{The rotation periods of late F, GK, and early M dwarfs are modeled
        with equation \ref{eqn:fgk_gyro}.}
    \item{The rotation periods of stars hotter than around 6250 K are
        described with a log-normal distribution with mean,
        $\log_{10}(P_\mathrm{rot})=0.56$, and an extra standard deviation,
        which added to observational uncertainties, of 0.5 dex.
    ...
    \item{The rotation periods of fully convective stars are described as a
         log-normal distribution with mean given by equation \ref{eqn:m_gyro},
         and additional standard deviation of 0.5 dex.
         This distribution reflects observations of late M dwarfs in the
        Praesepe cluster, which follow relation \ref{eqn:m_gyro}, with an
        additional standard deviation (in log-period) of around 0.5 dex.}
    ...
    \item{The rotation periods of subgiants are described with a log-normal
        distribution with mean given by equation \ref{eqn:fgk_gyro} and
        additional standard deviation of 0.5 dex.
        This is not a strictly accurate model of subgiant rotation periods
    ...
    \item{Finally, we model stars with very low and high metallicites (-3 $<$
        ...
        Rather than assume the same gyrochronology model can be naively
        applied to these stars, we take a more conservative approach and model
        them with a broad Gaussian distribution.
\end{itemize}
Inflating the variance for stars with non-Skumanich magnetic braking behavior
has two purposes: 1) in the cases of hot stars and fully convective stars, it
allows the broad distributions of rotation periods observed in clusters and
the field to be matched, and 2) it down-weights the age-information provided
by rotation periods in regions of the HRD/CMD where rotation periods are not
information-rich or the gyrochronology model is inaccurate or uncalibrated.}
