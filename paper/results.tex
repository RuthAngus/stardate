...
% Following our analysis of Praesepe which showed that rotation period
% uncertainties of around 10\% are realistic for cluster stars, we used 10\%
% uncertainties on the rotation periods, rather than the provided measurement
% uncertainties.
...
% The RMS of ages measured with the Praesepe-based gyrochronology model and
% isochrones via \sd\ is 3.4 Gyr and the RMS of ages measured with isochrones
% only is 13.5 Gyr.
% The median uncertainty of \sd\ ages is 1.4 Gyr and the median uncertainty of
% isochrone-only ages is 2.2 Gyr.
...
2.66 $\pm$ 0.21 Gyr.
% This is due to the \citet{angus15} model slightly under-predicting stellar
% rotation periods at this age, as shown in figure \ref{fig:NGC6819}.
...
% The ages quoted above are the {\it median} age of stars in the cluster,
% calculated using each method.
% It is worth noting however that the average, inverse variance-weighted  age of
% cluster stars is exactly 2.5 Gyr in each case.
...
age estimate.
% Overestimating the uncertainties can also result in inaccurate ages: 10\%
% rotation period uncertainties were too conservative and resulted in ages being
% affected by the prior and systematically over-estimated.
...
% NGC 6819 cluster (an open cluster with a precisely measured age from ensemble
% isochrone fitting and MS turn-off) demonstrates that combining isochrone
% fitting with gyrochronology provides more precise ages
