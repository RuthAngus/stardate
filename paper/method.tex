...
parameters that relate to age, but are not of interest in this study: \eep\
($E$), distance ($D$), V-band extinction ($A_V$) and the {\it inferred} bulk
metallicity, $F$.
...
\begin{equation} \label{eqn:factorized}
    p({\bf m_x}, T_{\mathrm{eff}}, \log(g), \hat{F}, \bar{\omega},
    C_{B-V}|A, E, D, A_V, F) ~p(P_\mathrm{rot}|A, C_{B-V}),
\end{equation}
...
For example, it is the \eep, age, metallicity, extinction and distance that
...
on stellar mass or \eep.
\eep, age and metallicity determine $C_{B-V}$, and $C_{B-V}$ along with age
...
%   - Priors
We place priors over the model parameters $A$, $E$, $F$, $D$ and $A_V$.
...
refer to the model parameters: age, \eep, distance, etc).
First, a set of parameters: age, \eep, true bulk metallicity, distance and
...
posterior PDF over age, \eep, metallicity, distance and extinction using the
...
Because this gyochronology is not calibrated to include stars turning off the
main sequence whose rotation slows rapidly due to their increasing radius, we
do not apply gyrochronology to stars with an \eep\ greater than
425\footnote{This number was determined by investigating the correspondance
between \eep\ and position on the HR diagram.
...
stars, as well as those with large \eep s.
...
${\bf \theta}$ is a vector of {\it parameters}: \eep, observed bulk
    metallicity, distance and V-band extinction; and ${\bf O}$ is a vector of
...
extinction, metallicity or \eep, only age and B-V color.
